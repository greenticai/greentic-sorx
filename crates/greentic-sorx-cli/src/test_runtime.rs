...
async function handleGreenticManagerSubmit(store, activity) {
  ...
  try {
    ...
    if (!submitResponse.ok) throw new Error('manager submit failed with HTTP ' + submitResponse.status);
    var target = value.manager_target || (value.record ? 'records/' + value.record : 'dashboard');
    var cardResponse = await fetch(String(submitUrl).replace(/\/submit(?:[?#].*)?$/, '/cards/' + target), {
      method: 'GET',
      headers: headers
    });
    if (!cardResponse.ok) throw new Error('manager card reload failed with HTTP ' + cardResponse.status);
    store.dispatch({
